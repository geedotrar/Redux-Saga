...
import EmployeeCreateForm from "../Form/Employees/EmployeeCreateForm";
import EmployeeEditForm from "../Form/Employees/EmployeeEditForm";
...
export default function EmployeeViewApi() {
  ...
  useEffect(() => {
    EmployeeApi.employeeList().then((data) => {
      setEmployee(data);
    });
    setRefresh(false);
  }, [refresh]);
                ...
                employee.map((e) => {
                  return (
                    <tr key={e.employeeId}>
                      ...
                      <td>{e.departmentId ? e.departmentId : "null"}</td>
                      <td>
